...
def IDEA_import(exportbestand):
    try:
        idea = win32ComClient.Dispatch(dispatch="Idea.IdeaClient")
        #RDF-definitie maken
        exportlocatie = exportbestand
        exportlocatie = exportlocatie[::-1].split("/",1)
        rdfpath = exportlocatie[1][::-1]
        rdfpath = rdfpath.replace("/","\\")+"\\"
        rdfnaam = exportlocatie[0][::-1].strip(".csv")
        rdfpath = rdfpath + rdfnaam + ".RDF"

        exportbestand = exportbestand.replace("/","\\")

        csvDefinition = idea.NewCsvDefinition()
        csvDefinition.DefinitionfilePath = rdfpath
        csvDefinition.CsvFilePath = exportbestand  
        csvDefinition.FieldDelimiter = ";"
        csvDefinition.TextEncapsulator = ""
        csvDefinition.FirstRowIsFieldNames = True
        idea.SaveCSVDefinitionFile(csvDefinition)
...
if __name__ == "__main__":
    ...
    if len(filenames) > 1:
        ...
        geconsolideerd.to_csv(exportbestand_geconsolideerd,
                            ...
                            decimal=",",
                            sep=";")
        IDEA_import(exportbestand_geconsolideerd)
